...
export default SIPMethods =>
  class MobileSessionDescriptionHandler extends EventEmitter {
    ...
    getDescription(options, modifiers) {
      this.logger.log('Getting description from SDH');
      options = options || {};
    ...
    setDescription(sessionDescription, options, modifiers) {
      ...
      return Promise.resolve()
        .then(
          function() {
            self.logger.log("Resolving setDescription")
    ...
    getDirection() {
      this.logger.error(`Get direction ${this.direction}`);
      return this.direction;
    }

    // Internal functions
    createOfferOrAnswer(RTCOfferOptions, modifiers) {
      this.logger.log('Create offer or answer');
      var self = this;
      var methodName;
    ...
    createRTCSessionDescriptionInit(RTCSessionDescription) {
      this.logger.log('createRTCSessionDescriptionInit');
      return {
    ...
    addDefaultIceCheckingTimeout(peerConnectionOptions) {
      this.logger.log('addDefaultIceCheckingTimeout');
    ...
    addDefaultIceServers(rtcConfiguration) {
      this.logger.log('addDefaultIceServers');
    ...
    checkAndDefaultConstraints(constraints) {
      this.logger.log('checkAndDefaultConstraints');
      let defaultConstraints = { audio: true, video: false };
    ...
    initPeerConnection(options) {
      ...
      this.peerConnection.oniceconnectionstatechange = function() {
        ...
        var stateEvent;
        self.logger.log(`onIceConnectionStateChange: ${this.iceConnectionState}`);
    ...
    acquire(constraints) {
      ...
      return new Promise(
        function(resolve, reject) {
           ...
           */
          console.log("sip acquiring local media")
          this.logger.log('acquiring local media');
          const self = this;
          this.emit('userMediaRequest', constraints);

          if (constraints.audio || constraints.video) {
            this.logger.log("Going to use getUserMedia...")
            this.logger.log(this.WebRTC.getUserMedia);
            ...
            this.WebRTC.getUserMedia(JSON.parse(JSON.stringify(constraints)))
              .then(
                function(streams) {
                  self.logger.log("Streams obtained from getUserMedia")
